...
import { formatAwardListTable } from '../formatters/table.js';
...
export function registerAwardCommand(program: Command) {
  program
    .command('award')
    .description('수상작 목록 조회')
    ...
    .action(async (opts: AwardOptions) => {
      const apiKey = resolveApiKey(opts);
      const client = createKopisClient(apiKey);

      try {
        const results = await client.getAwardList({
        ...
        console.log(opts.format === 'json' ? formatJson(results) : formatAwardListTable(results));
      } catch (err) {
