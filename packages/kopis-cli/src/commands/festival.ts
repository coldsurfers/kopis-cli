...
import { formatFestivalListTable } from '../formatters/table.js';
...
export function registerFestivalCommand(program: Command) {
  program
    .command('festival')
    .description('축제 목록 조회')
    ...
    .action(async (opts: FestivalOptions) => {
      const apiKey = resolveApiKey(opts);
      const client = createKopisClient(apiKey);

      try {
        const results = await client.getFestivalList({
        ...
        console.log(
          opts.format === 'json' ? formatJson(results) : formatFestivalListTable(results)
        );
      } catch (err) {
